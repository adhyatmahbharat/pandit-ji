...
export default function VendorComponent() {
  ...
  return (
    <Container maxWidth="xl">
      ...
      >
        <HeaderBreadcrumbs heading="Pandits" links={[{ name: 'Home', href: '/' }, { name: 'Pandits' }]} />
        <Box>
          <Grid container spacing={2} justifyContent="start" alignItems="center" mb={3}>
            {(isLoading ? Array.from(new Array(12)) : vendors).map((vendor, index) => (
              <React.Fragment key={vendor?.id || index}>
                <Grid size={{ lg: 3, md: 6, sm: 6, xs: 12 }}>
                  <VendorCard vendor={vendor} isLoading={isLoading} />
                </Grid>
              </React.Fragment>
            ))}
          </Grid>
          {!isLoading && !Boolean(vendors.length) && (
